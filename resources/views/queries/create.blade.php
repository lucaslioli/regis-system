...
@section('content')
    ...
    <div class="container">
        ...
        <form method="POST" action="/queries/{{ $query->id ?? 'store' }}" enctype="multipart/form-data">
            @csrf
            @isset($query->exists)
                @method('PUT')
            @endisset
            ...
            <div class="row">
                ...
                <div class="form-group col-4">
                    ...
                    <input class="form-control @error('qry_id') is-invalid @enderror" value="{{ old('qry_id', $query->qry_id ?? '') }}"
            ...
            <div class="form-row d-flex justify-content-end">
                <div class="col-md-4">
                    <button type="submit" class="btn btn-block btn-success"><i class="fas fa-plus"></i> Submit</button>
                </div>
            </div>
        ...
        @if(!isset($query->exists))
            ...
            <form method="POST" action="/queries/documents" enctype="multipart/form-data" id="queries-documents">
                ...
                <h3>Correlate queries with documents using JSON file</h3>
        ...
        <div id="response-text">
            @if(isset($response))
                ...
                @if(isset($queries))
                    Total of {{ $queries }} queries inserted. <br>
                @endif

                @if(isset($ignored) && $ignored != 0)
                    Total of {{ $ignored }} queries ignored.
                @endif
            @endif
        </div>
